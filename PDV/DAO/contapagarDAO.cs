    ...
    public class contapagarDAO
    {
        ...
        public DataTable ListarConta()
        {
            try
            {
                ...
                string sql = @"SELECT cp.*, f.nome FROM Conta_Pagar as cp INNER JOIN fornecedor as f on cp.Fornecedor_idFornecedor = f.idFornecedor ORDER BY valor asc";
                MySqlCommand cmd = new MySqlCommand(sql, con);
                con.Open();
                cmd.ExecuteNonQuery();
        ...
        public DataTable FiltrarContas(string pago, string naoPago)
        {
            ...
            {
                DataTable dt = new DataTable();
                string sql = @"SELECT * FROM Conta_Pagar WHERE pago IN ('" + pago + "','" + naoPago + "') ORDER BY valor asc";
                MySqlCommand cmd = new MySqlCommand(sql, con);
                con.Open();
                cmd.ExecuteNonQuery();
                MySqlDataAdapter da = new MySqlDataAdapter(cmd);
        ...
        public DataTable FiltrarContasAtrasadas(string dataAtual)
        {
            try
            {
                DataTable dt = new DataTable();
                string sql = @"SELECT * FROM Conta_Pagar WHERE data_vencimento < '" + dataAtual + "' AND pago != 'SIM' ORDER BY valor asc";
                MySqlCommand cmd = new MySqlCommand(sql, con);
                con.Open();
                cmd.ExecuteNonQuery();
                MySqlDataAdapter da = new MySqlDataAdapter(cmd);
